...
# 3. --mode 500 will set all status codes to 500. Such test suite is to be used for the jobs catching 500 errors.

# The basic logic here is: different combinations of required and optional parameters are created. This is based on the config_autogeneration_params.py.
# Next, different values are passed to the parameters according to the confid_autogeneration_param_values.py.
# Further, the cases are ordered alphabetically and passed to the output file. Each test case is mentioned only once in the test suite.

# Imports
import argparse
import itertools
from config_testsuite_param_values import *
from config_testsuite_params import *

# Parsing inputs from the command line
parser = argparse.ArgumentParser()

parser.add_argument(
    "--dc", help = "Choose the datacall to create the test suite")
parser.add_argument("--output", help = "Add the output filename")
parser.add_argument(
    "--mode", help = "Add 500 to create test suite for 500 errors. Values different to 500 will not have influence on the result.")
...
def creating_combs(resources):
    ...
    for combination_length in range(1, len(resources)+1):
            all_combs.extend(
                    list(itertools.combinations(resources, combination_length)))
    for req_param_combination in all_combs:
            all_combs_for_dc.extend(list(itertools.product(
            ...
            for group in resource_groups:
                for element in group:
                    ...
                    if len(optional_params_for_dc) > 0:
                        test_suite = test_suite.union(adding_opt_params(optional_params_for_dc, req_param_string, code, args.mode, vars()))
        # processing datacalls with more, than 1 required parameter
            ...
            for req_param_for_dc in all_combs_for_dc:
                ...
                for i in range(0, len(req_param_for_dc)):
                    ...
                    req_args_to_test_final = list(
                        itertools.product(*req_args_to_test))
                for req_args_ind in req_args_to_test_final:
                    code = 200
                    req_param_string = str()
                    req_param_for_dc_to_use = [
                        req_param_for_dc_ind[:-4] for req_param_for_dc_ind in req_param_for_dc]
                    codes = [req_param_for_dc_ind[-3:]
                             for req_param_for_dc_ind in req_param_for_dc]
                    if args.mode == "500":
                        code = 500
                    elif any(c == "400" for c in codes) or len(codes) < len(resource_params_for_dc):
                        code = 400
                    for i in list(zip(req_param_for_dc_to_use, req_args_ind)):
                        if len(req_param_string) < 1:
                            ...
                            if len(i[1]) > 0:
                                req_param_string += f"; {i[0]} = {i[1]}"
                    codes = [req_param_for_dc_ind[-3:] for req_param_for_dc_ind in req_param_for_dc]
                    if args.mode == "500":
                        code = 500
                    elif any(c == "400" for c in codes) or len(codes) < len(resource_params_for_dc):
                        code = 400
